...
int ion_alloc(struct ion_device_t *ion, unsigned long size, enum _ion_heap_type type,ion_buffer_t * *data)
{
        int err = 0;
...
static int ion_share(struct ion_device_t *ion, ion_buffer_t *data, int *share_fd)
{
        ...
        printf("%s: base %p, phys %lx, size %luK, fd %d, handle: %p", 
                        __FUNCTION__,  hnd->data.virt, hnd->data.phys, hnd->data.size/1024, *share_fd, hnd->handle);
        return err;
}
